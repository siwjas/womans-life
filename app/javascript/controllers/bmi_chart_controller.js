...
export default class extends Controller {
  ...
  connect() {
    console.log("BMI chart controller connected")
    ...
    this.element.addEventListener('bmi-chart:initialize', this.handleInitialize.bind(this))
  }
  ...
  handleInitialize() {
    console.log("BMI chart initialize event received")
  ...
  renderBmiChart() {
    // Implementar a renderização do gráfico de IMC aqui
    // Pode ser similar ao gráfico de peso, mas com categorias de IMC
    ...
    console.log("BMI Chart data:", data)
    
    // Encontrar valores mínimos e máximos para escala
    const bmis = data.map(item => parseFloat(item.bmi))
    const minBmi = Math.max(0, Math.floor(Math.min(...bmis) * 0.95))
    const maxBmi = Math.ceil(Math.max(...bmis) * 1.05)
    ...
    header.innerHTML = `
      <div>
        ...
        <span class="ml-2 text-gray-400">Min: ${minBmi} | Max: ${maxBmi}</span>
      </div>
      <span>Data</span>
    `
    ...
    const steps = 5
    const stepSize = (maxBmi - minBmi) / steps
    for (let i = steps; i >= 0; i--) {
      const value = Math.round((minBmi + stepSize * i) * 10) / 10
    ...
    const firstBmi = parseFloat(data[0].bmi)
    const lastBmi = parseFloat(data[data.length - 1].bmi)
    const trend = lastBmi > firstBmi ? 'up' : lastBmi < firstBmi ? 'down' : 'stable'
    
    // Criar barras para cada ponto de dados
    data.forEach((item, index) => {
      // Garantir que o IMC seja um número
      const bmi = parseFloat(item.bmi)
      
      // Calcular a posição vertical da barra
      const topPercentage = 100 - ((bmi - minBmi) / (maxBmi - minBmi)) * 100
      
      // Determinar cor com base na categoria de IMC
      let barColor = 'bg-blue-200'  // Abaixo do peso
      if (bmi >= 18.5 && bmi < 25) {
        barColor = 'bg-green-200'   // Peso normal
      } else if (bmi >= 25 && bmi < 30) {
        barColor = 'bg-yellow-200'  // Sobrepeso
      } else if (bmi >= 30) {
        barColor = 'bg-red-200'     // Obesidade
      }
      ...
      barElement.className = `absolute w-full ${barColor} hover:opacity-80 rounded-t cursor-pointer transition-all duration-200 flex flex-col items-center`
      barElement.style.top = `${topPercentage}%`
      barElement.style.bottom = '0'
      
      // Adicionar indicador de tendência no topo da barra
      if (index > 0) {
        const prevBmi = parseFloat(data[index - 1].bmi)
    ...
    trendSummary.className = 'mt-8 text-xs text-center'
    
    if (trend === 'up') {
      const diff = (lastBmi - firstBmi).toFixed(1)
      const percentage = ((lastBmi - firstBmi) / firstBmi * 100).toFixed(1)
      trendSummary.innerHTML = `
        <span class="text-red-500 font-medium">Tendência: Aumento de ${diff} no IMC (${percentage}%)</span>
      `
    } else if (trend === 'down') {
      const diff = (firstBmi - lastBmi).toFixed(1)
      const percentage = ((firstBmi - lastBmi) / firstBmi * 100).toFixed(1)
      trendSummary.innerHTML = `
        <span class="text-green-500 font-medium">Tendência: Redução de ${diff} no IMC (${percentage}%)</span>
      `
    } else {
      trendSummary.innerHTML = `
        <span class="text-blue-500 font-medium">Tendência: IMC Estável</span>
      `
    }
  ...
  disconnect() {
    ...
    this.element.removeEventListener('bmi-chart:initialize', this.handleInitialize.bind(this))
  }
} 